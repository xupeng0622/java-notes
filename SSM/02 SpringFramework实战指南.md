...
6.  三层架构IoC配置
    ...
    <beans xmlns="http://www.springframework.org/schema/beans"
        ...
        <bean id="studentService" class="com.sdu.service.impl.StudentServiceImpl">
...
5.  三层架构IoC配置类
    ```java
    @Configuration
    @ComponentScan(basePackages = "com.sdu")
...
3.  标记IoC注解：@Component,@Service,@Controller,@Repository&#x20;
4.  标记DI注解：@Autowired @Qualifier @Resource @Value
5.  \<context:component-scan标签指定注解范围使用@ComponentScan(basePackages = {"com.sdu.components"})替代
...
4.  **代理总结**
    ...
    **代理方式可以解决附加功能代码干扰核心代码和不方便统一维护的问题！**
    ...
    但是实际开发中，不需要编写代理代码，我们可以使用\[Spring AOP]框架，他会简化动态代理的实现！！！
...
4.  声明切面类
    ...
    public class LogAspect {
        ...
        @Before(value = "execution(public int com.sdu.proxy.CalculatorPureImpl.add(int,int))")
        public void printLogBeforeCore() {
            System.out.println("[AOP前置通知] 方法开始了");
        }
        
        @AfterReturning(value = "execution(public int com.sdu.proxy.CalculatorPureImpl.add(int,int))")
        public void printLogAfterSuccess() {
            System.out.println("[AOP返回通知] 方法成功返回了");
        }
        
        @AfterThrowing(value = "execution(public int com.sdu.proxy.CalculatorPureImpl.add(int,int))")
        public void printLogAfterException() {
            System.out.println("[AOP异常通知] 方法抛异常了");
        }
        
        @After(value = "execution(public int com.sdu.proxy.CalculatorPureImpl.add(int,int))")
...
1.  **JointPoint接口**
    ...
    @Before(value = "execution(public int com.sdu.aop.api.Calculator.add(int,int))")
...
2.  **方法返回值**
    ...
    @AfterReturning(
            value = "execution(public int com.sdu.aop.api.Calculator.add(int,int))",
...
2.  **切点表达式语法**
    ...
    -   第四位：指定包的地址
        ```java
         固定的包: com.sdu.api | service | dao
         单层的任意命名: com.sdu.*  = com.sdu.api  com.sdu.dao  * = 任意一层的任意命名
         任意层任意命名: com.. = com.sdu.api.erdaye com.a.a.a.a.a.a.a  ..任意层,任意命名 用在包上!
...
2.  同一类内部引用
    ...
    ```java
    // 切入点表达式重用
    @Pointcut("execution(public int com.sdu.aop.api.Calculator.add(int,int)))")
...
3.  不同类中引用
    ...
    ```java
    @Before(value = "com.sdu.spring.aop.aspect.LogAspect.declarPointCut()")
...
4.  切点统一管理
    ...
    ```java
    @Component
    public class sduPointCut {
        
        @Pointcut(value = "execution(public int *..Calculator.sub(int,int))")
        public void sduGlobalPointCut(){}
        
        @Pointcut(value = "execution(public int *..Calculator.add(int,int))")
        public void sduSecondPointCut(){}
...
```java
// 使用@Around注解标明环绕通知方法
@Around(value = "com.sdu.aop.aspect.sduPointCut.transactionPointCut()")
...
public class EmployeeService {
    ...
    public void getEmpList() {
       System.out.print("方法内部 com.sdu.aop.imp.EmployeeService.getEmpList");
    }
}
```
...
2.  配置Spring配置文件
    ```xml
    <!-- 配置目标类的bean -->
    <bean id="calculatorPure" class="com.sdu.aop.imp.CalculatorPureImpl"/>
        
    <!-- 配置切面类的bean -->
    <bean id="logAspect" class="com.sdu.aop.aspect.LogAspect"/>
...
1.  配置事务管理器
    ...
    public class DataSourceConfig {
         ...
         */
        @Bean
        public DataSource dataSource(@Value("${sdu.url}")String url,
                                     @Value("${sdu.driver}")String driver,
                                     @Value("${sdu.username}")String username,
                                     @Value("${sdu.password}")String password){
            DruidDataSource dataSource = new DruidDataSource();
            dataSource.setDriverClassName(driver);
            dataSource.setUrl(url);
            dataSource.setUsername(username);
